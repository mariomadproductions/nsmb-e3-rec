BackgroundSwapper_ctor:
	STMFD	SP!, {R4, LR}
	LDR     R0, =BackgroundSwapper_ActorSize			@struct size
	LDR     R0, [R0]
	BL		NSMB_AllocFromGameHeap
	MOVS	R4, R0
	BEQ		BackgroundSwapper_ctor_end
	BL		EnemyActor_ctor
	LDR		R0, =BackgroundSwapper_vtable
	STR		R0, [R4]

BackgroundSwapper_ctor_end:
	MOV		R0, R4
	LDMFD	SP!, {R4, PC}

BackgroundSwapper_null:
	BX		LR

BackgroundSwapper_beforeDraw:
	B		enemyActor_beforeDraw
	BX		LR

BackgroundSwapper_init:
	STMFD	SP!, {R4, LR}
	MOV		R4, R0
	...
	MOV	R0, R4
	BL		BackgroundSwapper_OnCreate

	MOV	R0, #1
	LDMFD	SP!, {R4, PC}

BackgroundSwapper_beforeDelete:
	B		actor_beforeDelete

@BackgroundSwapper_delete:
@	B		base_onDelete

BackgroundSwapper_vtable:
	.long BackgroundSwapper_init
	.long actor_beforeCreate
	.long actor_afterCreate
	.long BackgroundSwapper_OnDelete
	.long BackgroundSwapper_beforeDelete
	.long actor_afterDelete
	.long BackgroundSwapper_OnExecute
	.long enemyActor_beforeExecute
	.long enemyActor_afterExecute
	.long BackgroundSwapper_OnDraw
	...
	.long BackgroundSwapper_null	@Unknown
	...
	.long BackgroundSwapper_null	@End Of Level Callback aka EOL
	.long enemyActor_virt55		@Lava Callback
	.long enemyActor_virt56		@Makes enemy jump
	.long BackgroundSwapper_null	@Nothing?
	...
	.long BackgroundSwapper_null	@@Collision with another enemy - Doesnt trigger virt64 or virt65
...
BackgroundSwapper_Profile:
	.int   BackgroundSwapper_ctor
	.short 0xFB
	.short 0x69
	.int   0
	.int   0
